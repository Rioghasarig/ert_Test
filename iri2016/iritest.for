...
c          jf(4)=.false.      ! t=B0table f=other models (f)
c          jf(5)=.false.      ! t=CCIR  f=URSI foF2 model (f)
c          jf(6)=.false.      ! t=DS95+DY85   f=RBV10+TBT15 (f)
...
c          jf(23)=.false.     ! t=AEROS/ISIS f=TTS Te with PF10.7 (f)
...
c          jf(30)=.false.     ! t=corr f=NeQuick topside (f)
...
c          jf(35)=.false. 	  ! t=auroral E-storm model on f=off (f)
...
c          jf(39)=.false. 	  ! t=M3000F2 model f=new hmF2 models (f)
...
c          jf(47)=.false. 	  ! t=CGM on  f=CGM off
        else
